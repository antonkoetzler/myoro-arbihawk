...
class BettingService:
    ...
    def place_bets_for_model(self, market: str, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if not model_path:
                result["error"] = "No model path in active version"
                if self.log_callback:
                    self.log_callback("error", result["error"])
                return result
            if not model_path.exists():
                # Fallback: same models dir as train.py (package root/models/saved)
                fallback = Path(__file__).resolve().parent.parent / "models" / "saved" / model_path.name
                if fallback.exists():
                    model_path = fallback
                else:
