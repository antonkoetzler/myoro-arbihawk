...
Usage:
    python scripts/import_data.py <export_file.zip> [--overwrite-db] [--overwrite-models] [--overwrite-config]
"""
...
def check_schema_compatibility(export_version: dict, current_db: Database) -> bool:
    ...
    if export_schema > current_schema:
        print(
            f"INFO: Export schema version ({export_schema}) is newer than current ({current_schema})"
        )
        print("      Database migrations will be applied automatically")
        return True
    elif export_schema < current_schema:
        print(
            f"WARNING: Export schema version ({export_schema}) is older than current ({current_schema})"
        )
